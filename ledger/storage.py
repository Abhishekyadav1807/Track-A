...
def connect(path):
    ...
    db.executescript('''
        ...
        CREATE UNIQUE INDEX IF NOT EXISTS idx_invoices_key ON invoices(customer_id, invoice_number);
    ''')
    return db
...
def insert_invoice(db, row):
    ...
    if old:
        if round(float(old['amount']), 2) == round(float(row['amount']), 2) and old['due_date'] == row['due_date']:
            return 'skipped'
